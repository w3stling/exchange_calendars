...
class XNZECalendarTestCase(ExchangeCalendarTestBase, TestCase):
    ...
    def test_christmas_and_boxing_day_special_cases(self):
        ...
        # trading day.
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2010-12-24", tz=UTC)))
        self.assertEqual(
            self.calendar.session_close(pd.Timestamp("2010-12-24", tz=UTC)),
            pd.Timestamp("2010-12-24 16:45", tz="NZ"),
        )

        # Saturday, but also Christmas Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2010-12-25", tz=UTC)))
        # Sunday, but also Boxing Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2010-12-26", tz=UTC)))
        # Since both Christmas and Boxing Day fell on the weekend, 2
        # additional holidays are observed.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2010-12-27", tz=UTC)))
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2010-12-28", tz=UTC)))
        ...
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2011-12-23", tz=UTC)))
        self.assertEqual(
            self.calendar.session_close(pd.Timestamp("2011-12-23", tz=UTC)),
            pd.Timestamp("2011-12-23 12:45", tz="NZ"),
        )

        # Saturday.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-12-24", tz=UTC)))
        # Sunday, but also Christmas Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-12-25", tz=UTC)))
        # Boxing Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-12-26", tz=UTC)))
        # Since Christmas was on a Sunday, an additional holiday is
        # observed the day after Boxing Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-12-27", tz=UTC)))
    ...
    def test_new_years_day_and_day_after_special_cases(self):
        ...
        # trading day.
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2010-12-31", tz=UTC)))
        self.assertEqual(
            self.calendar.session_close(pd.Timestamp("2010-12-31", tz=UTC)),
            pd.Timestamp("2010-12-31 16:45", tz="NZ"),
        )

        # Saturday, but also New Year's Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-01-01", tz=UTC)))
        # Sunday, but also the day after New Year's Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-01-02", tz=UTC)))
        # Since both New Year's and the day after fell on the weekend, 2
        # additional holidays are observed.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-01-03", tz=UTC)))
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-01-04", tz=UTC)))
        ...
        # half day (beginning in 2011.)
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2011-12-30", tz=UTC)))
        self.assertEqual(
            self.calendar.session_close(pd.Timestamp("2011-12-30", tz=UTC)),
            pd.Timestamp("2011-12-30 12:45", tz="NZ"),
        )

        # Saturday.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2011-12-31", tz=UTC)))
        # Sunday, but also New Year's Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2012-01-01", tz=UTC)))
        # The day after New Year's Day.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2012-01-02", tz=UTC)))
        # Since New Year's Day was on a Sunday, an additional holiday is
        # observed two days later.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2012-01-03", tz=UTC)))
    ...
    def test_weekend_to_monday_holidays(self):
        ...
        # for Waitangi Day.
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2010-02-08", tz=UTC)))
        ...
        # for Anzac Day.
        self.assertTrue(self.calendar.is_session(pd.Timestamp("2010-04-26", tz=UTC)))
        ...
        # following Monday.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2016-02-08", tz=UTC)))
        ...
        # following Monday.
        self.assertFalse(self.calendar.is_session(pd.Timestamp("2015-04-27", tz=UTC)))
